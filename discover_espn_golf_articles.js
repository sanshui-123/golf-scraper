 ...
 * 
 * 使用方法：
 * node discover_espn_golf_articles.js 50 --urls-only
 */
...
// 输出文件
const OUTPUT_FILE = 'deep_urls_espn_golf.txt';
const DETAILS_FILE = 'espn_golf_details.json';
...
function isGolfContent(url, title = '') {
    ...
    // 必须是文章URL - ESPN使用特殊的URL格式
    const isArticle = urlLower.includes('/story/_/id/') || 
                     urlLower.includes('/insider/') ||
                     urlLower.includes('/golf/news/');
    ...
    try {
        ...
        const articles = await page.evaluate(() => {
            ...
            const selectors = [
                ...
                '.contentItem a[href]',
                '.headline a[href]',
                '.headlineStack a[href]',
                '.Card__Content a[href]',
                '.MediaList__item a[href]',
                'h1 a[href]',
                'h2 a[href]',
                'h3 a[href]'
            ];
            ...
            selectors.forEach(selector => {
                document.querySelectorAll(selector).forEach(a => {
                    ...
                    if (href && 
                        href.startsWith('https://www.espn.com') && 
                        !seenUrls.has(href) &&
                        text.length > 10) {
                        ...
                        // 获取发布时间（如果有）
                        let time = '';
                        const parent = a.closest('article') || a.closest('.contentItem');
                        if (parent) {
                            const timeEl = parent.querySelector('time, .timestamp, .date');
                            if (timeEl) {
...
// 主函数
async function discoverESPNGolfArticles() {
    console.log('🏌️ ESPN Golf 内容发现');
    ...
    try {
        ...
        // 要扫描的页面
        const pagesToScan = [
            { url: 'https://www.espn.com/golf/', name: 'Golf主页' },
            { url: 'https://www.espn.com/golf/pga', name: 'PGA Tour' },
            { url: 'https://www.espn.com/golf/lpga', name: 'LPGA' },
            { url: 'https://www.espn.com/golf/european', name: '欧巡赛' },
            { url: 'https://www.espn.com/golf/masters', name: '大师赛' },
            { url: 'https://www.espn.com/golf/usopen', name: '美国公开赛' },
            { url: 'https://www.espn.com/golf/theopen', name: '英国公开赛' },
            { url: 'https://www.espn.com/golf/pgachampionship', name: 'PGA锦标赛' }
        ];
...
(async () => {
    try {
        await discoverESPNGolfArticles();
        console.log('\n✅ 完成！');
        process.exit(0);
    } catch (error) {
